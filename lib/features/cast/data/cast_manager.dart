...
class CastManager extends ChangeNotifier {
    ...
    switch (evt.kind) {
      ...
      case 'load_failed':
        ...
        if (_state == CastState.connecting ||
          ...
          _errorMessage = AppL10n.current.castErrorTvRefusedStream;
  ...
  Future<void> castTo(
    ...
    try {
      ...
      _state = CastState.error;
      _errorMessage = AppL10n.current.castErrorTooLong;
      _setProgress(
        CastProgress.failure(
          AppL10n.current.castErrorTvTooSlow,
  ...
  Future<void> _castToInner(
    ...
    try {
      ...
      final String userMessage = relayPathBlocked
          ? AppL10n.current.castErrorRelayBlocked
  ...
  String friendlyMessageFor(Exception e) {
    ...
    if (s.contains('google play services') ||
      ...
      return AppL10n.current.castErrorNoPlayServices;
    }
    ...
    if (s.contains('hostname') ||
        ...
        s.contains('nodename') ||
        s.contains('servname')) {
      return AppL10n.current.castErrorDnsUnstable;
    }
    ...
    if (s.contains('no route to host') ||
      ...
      return AppL10n.current.castErrorTvUnreachableChecklist;
    }

    // --- Reseau bas-niveau ---
    if (s.contains('réseau') || s.contains('socket')) {
      return AppL10n.current.castErrorTvConnectionFailed;
    }

    return AppL10n.current.castErrorGeneric;
  }
  ...
  void _updateRelayKeepAlive(String title) {
    if (_currentRelayUrl != null) {
      ...
      unawaited(PipService.instance.startBackgroundAudio(
        title,
        body: AppL10n.current.castNotifRelayKeepAlive,
      ));
    } else {
      _stopRelayKeepAlive();
    }
