...
class _AdminPinScreenState extends State<AdminPinScreen> {
  ...
  Future<void> _handleComplete() async {
    // Libellés capturés AVANT les await : le context du State reste
    // utilisable, mais on suit le pattern « capture avant await ».
    final String mismatchMsg = context.l10n.adminPinMismatch;
    final String tooManyMsg = context.l10n.adminPinTooManyAttempts;
    final String wrongMsg = context.l10n.lockPinWrong;
    if (_isCreating) {
      if (!_confirming) {
      ...
      if (ok) {
        ...
        _failCount++;
        setState(() {
          _error = _failCount >= 3 ? tooManyMsg : wrongMsg;
          _entered = '';
        });
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(context.l10n.adminPinAccessTitle),
      ),
      body: SafeArea(
        child: Center(
