...
class _AdminClientEditSheetState extends State<AdminClientEditSheet> {
  ...
  String? _validate() {
    ...
    // Le nom de playlist est optionnel — on met le nom par défaut
    // (cf. playlistDefaultIptv) si vide. Permet à l'admin de juste
    // taper MAC + URL.
    if (_type == 'm3u') {
      if (_m3uUrlCtrl.text.trim().isEmpty) {
        return context.l10n.adminM3uUrlRequired;
      }
    } else {
  ...
  void _save() {
    ...
    // Nom par défaut LOCALISÉ à la création : ce nom est ENREGISTRÉ tel
    // quel côté serveur puis poussé à l'app cliente — il est donc figé
    // dans la langue active de l'ADMIN au moment de la saisie (comme un
    // nom de fichier par défaut). Impossible de localiser à l'affichage :
    // c'est un champ libre que l'admin peut renommer.
    final String pName = _playlistNameCtrl.text.trim().isEmpty
        ? context.l10n.playlistDefaultIptv
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: context.l10n
                        .adminPlaylistNameHint(context.l10n.playlistDefaultIptv),
                  ),
                ),
                const SizedBox(height: 14),
                if (_type == 'm3u') ...<Widget>[
