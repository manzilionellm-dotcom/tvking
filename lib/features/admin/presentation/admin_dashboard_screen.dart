...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _load() async {
    final AdminCredentials cred = AdminCredentials.instance;
    if (!cred.canWrite) {
      setState(() => _error = context.l10n.adminMissingCredentials);
      return;
    }
    setState(() {
      _loading = true;
    ...
    if (_clients.any((AdminClient c) => c.mac == created.mac)) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(
            context.l10n.adminClientDuplicateMac,
            style: AppTextStyles.bodyMedium,
          ),
        ),
      );
        ...
        children: <Widget>[
          Row(
            children: <Widget>[
              ...
              const SizedBox(width: 8),
              Text(context.l10n.adminServerConnectionTitle,
          ...
          const SizedBox(height: 10),
          Text(
            context.l10n.adminNotDeployedHelp,
            style: AppTextStyles.bodyMedium.copyWith(
              fontSize: 11,
              color: AppColors.textMuted,
              height: 1.4,
