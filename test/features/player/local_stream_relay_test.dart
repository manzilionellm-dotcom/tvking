...
void main() {
  ...
  // =================================================================
  //  Anti-SSRF : /s?u= et /hls?u= refusent toute URL non enregistrée,
  //  les schémas hors http(s), le loopback non listé et les métadonnées.
  // =================================================================
  group('anti-SSRF allowlist', () {
    Future<Uri> _relayOrigin() async {
      final String localUrl =
          await LocalStreamRelay.instance.playUrlFor(fakeUrl);
    ...
    Future<HttpClientResponse> _get(Uri uri) async {
      ...
      return (await client.getUrl(uri)).close();
    }

    Uri _u(Uri origin, String path, String upstream) => Uri(
          scheme: origin.scheme,
          host: origin.host,
          port: origin.port,
          path: path,
          queryParameters: <String, String>{'u': upstream},
        );

    test('/s?u= URL non enregistrée → 403, zéro fetch amont', () async {
      final Uri origin = await _relayOrigin();
      final int before = upstreamConnections;
      final HttpClientResponse resp = await _get(
          _u(origin, '/s', 'http://evil.example/live/U/P/1.ts'));
      expect(resp.statusCode, HttpStatus.forbidden);
      await resp.drain<void>();
      expect(upstreamConnections, before,
          reason: 'une URL non enregistrée ne doit PAS être fetchée');
    });

    test('/s?u= file:// → 403', () async {
      final Uri origin = await _relayOrigin();
      final HttpClientResponse resp =
          await _get(_u(origin, '/s', 'file:///etc/passwd'));
      expect(resp.statusCode, HttpStatus.forbidden);
      await resp.drain<void>();
    });

    test('/s?u= métadonnées 169.254.169.254 → 403', () async {
      final Uri origin = await _relayOrigin();
      final HttpClientResponse resp = await _get(
          _u(origin, '/s', 'http://169.254.169.254/latest/meta-data/'));
      expect(resp.statusCode, HttpStatus.forbidden);
      await resp.drain<void>();
    });

    test('/s?u= loopback NON enregistré → 403', () async {
      final Uri origin = await _relayOrigin();
      final HttpClientResponse resp =
          await _get(_u(origin, '/s', 'http://127.0.0.1:1/secret'));
      expect(resp.statusCode, HttpStatus.forbidden);
      await resp.drain<void>();
    });

    test('/hls?u= URL non enregistrée → 403', () async {
      final Uri origin = await _relayOrigin();
      final HttpClientResponse resp = await _get(
          _u(origin, '/hls', 'http://evil.example/live/U/P/1.m3u8'));
      expect(resp.statusCode, HttpStatus.forbidden);
      await resp.drain<void>();
    });

    test('URL enregistrée par playUrlFor → le flux passe toujours', () async {
      final LocalStreamRelay relay = LocalStreamRelay.instance;
      final String localUrl = await relay.playUrlFor(fakeUrl);
      final HttpClient client = HttpClient();
      final HttpClientResponse resp =
    ...
    test('même enregistrée, 169.254.169.254 reste 403', () async {
      final LocalStreamRelay relay = LocalStreamRelay.instance;
      final String localUrl =
          await relay.playUrlFor('http://169.254.169.254/latest/meta-data/');
      final HttpClientResponse resp = await _get(Uri.parse(localUrl));
      expect(resp.statusCode, HttpStatus.forbidden);
      await resp.drain<void>();
    });

    test('sous-playlist HLS réécrite → URL abs autorisée', () async {
      ...
      final String master = 'http://127.0.0.1:${panel.port}/master.m3u8';
      final LocalStreamRelay relay = LocalStreamRelay.instance;
      final String local = await relay.hlsPlaylistUrlFor(master);
      final HttpClient client = HttpClient();
      addTearDown(() {
        client.close(force: true);
      });
      final HttpClientResponse masterResp =
          await (await client.getUrl(Uri.parse(local))).close();
      expect(masterResp.statusCode, 200);
      final String body =
          await masterResp.transform(const Utf8Decoder()).join();
      final String subLine = body.split('\n').firstWhere(
            (String l) => l.startsWith('http://127.0.0.1') && l.contains('/hls?'),
          );
      final HttpClientResponse subResp =
          await (await client.getUrl(Uri.parse(subLine))).close();
      expect(subResp.statusCode, 200,
          reason: 'rewriteSubPlaylist doit enregistrer l\'URL abs');
      await subResp.drain<void>();
    });
  });

  // =================================================================
  //  _short() masque USER/PASS AVANT troncature — jamais en clair
  //  dans les traces kDebugMode.
  // =================================================================
  test('les traces debug masquent USER/PASS (jamais en clair)', () async {
    ...
    final String credUrl =
        'http://127.0.0.1:${fakeUpstream.port}/live/SECRETUSER/SECRETPASS/42.ts';
    final LocalStreamRelay relay = LocalStreamRelay.instance;
    final String localUrl = await relay.playUrlFor(credUrl);
    final HttpClient client = HttpClient();
    final HttpClientResponse resp =
        await (await client.getUrl(Uri.parse(localUrl))).close();
    int bytes = 0;
    ...
    // Zap → debugPrint de fermeture via _short.
    await relay.playUrlFor(fakeUrl);
    await Future<void>.delayed(const Duration(milliseconds: 150));

    final String all = logs.join('\n');
    expect(all, isNot(contains('SECRETUSER')),
