...
function renderProfiles(mac, list, boxId) {
  ...
  let h = '<table class="profiles"><tr>' +
    ...
    '<th>Categories bloquees</th></tr>';
  list.forEach((p, i) => {
    h += '<tr data-id="' + p.id + '">' +
      '<td>' + p.emoji + ' ' + p.name + '</td>' +
      '<td><input type="checkbox" class="pEnabled"' + (p.enabled ? ' checked' : '') + '></td>' +
      '<td><input type="checkbox" class="pKids"' + (p.kids ? ' checked' : '') + '></td>' +
      // Le PIN existant ne se relit pas : on n'a que son empreinte. Le
      // champ reste VIDE et ne change rien tant qu'on n'y touche pas —
      // sinon ouvrir la fiche puis enregistrer effacerait tous les codes.
      '<td><input class="pPin" maxlength="8" placeholder="' +
        (p.pin ? 'code pose' : 'aucun') + '" style="width:90px"></td>' +
      '<td><input class="pCats" style="width:220px" placeholder="Adulte, XXX" value="' +
        (p.blockedCategories || []).join(', ') + '"></td>' +
      '</tr>';
  });
  h += '</table>' +
...
async function saveProfiles(mac, boxId) {
  ...
  rows.forEach((tr) => {
    ...
    const o = {
      ...
      name: tr.cells[0].textContent.trim().replace(/^\S+\s/, ''),
      emoji: tr.cells[0].textContent.trim().split(' ')[0],
      enabled: tr.querySelector('.pEnabled').checked,
      kids: tr.querySelector('.pKids').checked,
      blockedCategories: tr.querySelector('.pCats').value
        .split(',').map((x) => x.trim()).filter(Boolean),
    };
    // Champ vide = on ne touche pas au code. « - » = on l'efface.
    if (pin === '-') o.pin = '';
  ...
  try {
    ...
    msg.textContent = d.ok
      ? ('Enregistre (' + d.count + ' profils) — la box se met a jour toute seule.')
...
function renderDetail(c) {
  ...
  // --- PROFILS FAMILLE (chargés à la demande) ---
  //  Volontairement PAS chargés avec la fiche : ils demandent une requête
  //  de plus, et on ouvre beaucoup de fiches juste pour vérifier une date
  //  d'abonnement. Un bouton, et ils arrivent.
  g += '<div class="grp"><h3>Profils famille</h3>' +
       '<div id="profilesBox"><button class="ghost" ' +
       'onclick="loadProfiles(\\'' + c.mac + '\\')">Afficher les 5 profils</button>' +
       '</div></div>';
...
async function handleInviteRedeem(request, env) {
  ...
  const prev = await env.DB
    ...
    .bind(mac).first();
  // Invitations UTILISÉES par l'émetteur sur les 7 derniers jours (quota
  // hebdo glissant de 5, qui se renouvelle).
  const weekAgo = now - INVITE_WEEK_MS;
  const usedCnt = issuer ? await env.DB
    .prepare(
      'SELECT COUNT(*) AS n FROM app_invites ' +
      'WHERE issuer_mac = ? AND redeemed_at IS NOT NULL AND redeemed_at > ?',
...
const PROFILE_PIN_ITERATIONS = 20000;

async function profilePinHash(pin, salt) {
...
async function ensureDeviceProfilesTable(env) {
  try {
    await env.DB.prepare(
...
async function readDeviceProfiles(env, MAC, { create = true } = {}) {
  if (!env.DB) return null;
...
async function writeDeviceProfiles(env, MAC, list) {
  await ensureDeviceProfilesTable(env);
  await env.DB.prepare(
    `INSERT INTO device_profiles (mac, profiles_json, updated_at)
     VALUES (?, ?, ?)
...
//  GET|PUT /admin/profiles/:mac — ce que le PANEL utilise.
//
//  Protégé par le secret admin, comme le reste de /admin/*.
//
//  Le PUT accepte un `pin` EN CLAIR (« 1234 ») : c'est ce que l'admin
//  tape. On le transforme ici en empreinte salée avant de l'écrire, et
//  il ne ressort jamais. `pin: ""` efface le code ; `pin` absent laisse
//  le code existant tel quel — sans cette distinction, ouvrir puis
//  enregistrer la fiche effacerait tous les PIN sans prévenir.
async function handleAdminProfiles(request, env, mac) {
  if (!MAC_RX.test(mac)) return badRequest('invalid mac');
  ...
  if (request.method === 'GET') {
    ...
    return json({ mac: MAC, profiles: (list || []).map(({ _order, ...p }) => p) });
  }
  if (request.method !== 'PUT') return badRequest('only GET/PUT');

  let body;
  try { body = await request.json(); } catch (_) { return badRequest('bad json'); }
  const incoming = Array.isArray(body && body.profiles) ? body.profiles : null;
  if (!incoming) return badRequest('profiles[] requis');
  if (incoming.length > 12) return badRequest('12 profils maximum');

  const existing = (await readDeviceProfiles(env, MAC, { create: false })) || [];
  ...
  for (let i = 0; i < incoming.length; i++) {
    ...
    if (typeof raw.pin === 'string') {
      ...
      if (!clear) {
        norm.pin = null;                       // effacement explicite
      } else if (/^[0-9]{4,8}$/.test(clear)) {
        const salt = newProfileSalt();
        norm.pin = { salt, hash: await profilePinHash(clear, salt) };
      } else {
        return badRequest(`PIN invalide pour « ${norm.name} » : 4 à 8 chiffres`);
      }
    } else {
      norm.pin = before ? before.pin : null;   // inchangé
    }
    delete norm._order;
    out.push(norm);
  }
  if (!out.length) return badRequest('aucun profil valide');

  await writeDeviceProfiles(env, MAC, out);
  return json({ ok: true, mac: MAC, count: out.length });
}

//  GET /api/device-profiles/:mac — ce que l'APP interroge.
//
//  Publique comme /api/device-source : l'app n'a pas de secret. Ce
//  qu'elle reçoit ne contient AUCUN code en clair, seulement des
//  empreintes — c'est ce qui rend cette route sûre sans jeton.
async function handlePublicDeviceProfiles(env, mac) {
  ...
  if (!list) {
    ...
    // profils sans PIN effacerait ceux du panel à la moindre panne D1.
    return json({ mac: MAC, profiles: [], available: false });
  }
  return json({
    mac: MAC,
...
async function handleRequest(request, env, ctx) {
    ...
    //
    // Publique comme device-source : l'app n'a pas de secret. Ce qu'elle
    // recoit ne contient AUCUN code en clair, seulement des empreintes —
    // c'est ce qui rend cette route sure sans jeton.
    if (segments[0] === 'api' && segments[1] === 'device-profiles' &&
        segments.length === 3) {
      if (request.method !== 'GET') {
    ...
    // /admin/profiles/:mac — GET pour lire, PUT pour enregistrer.
    // Le panel y envoie le PIN EN CLAIR ; il est transforme en empreinte
    // avant ecriture et ne ressort jamais.
    if (segments[0] === 'admin' && segments[1] === 'profiles' &&
        segments.length === 3) {
      if (!checkAdmin(request, env)) return unauthorized();
      return await handleAdminProfiles(request, env, segments[2]);
    }
    ...
    // =========================================================
    //  UNE SEULE ADRESSE DE TÉLÉCHARGEMENT : /tv
    // =========================================================
    //  Décision du propriétaire (30/08) : « efface tout, tous les liens
    //  qui existent, on reste avec une seule ».
    //
    //  Il avait raison. On en était à QUATORZE : /tv, /defewtv, /tvbox,
    //  /defew, /777, /7777, /tv7, /7tv, /seventv, /sevenmotion, /apk,
    //  /d, /direct, /install. Chacune se justifiait au moment où on
    //  l'ajoutait ; ensemble elles rendaient impossible de répondre en
    //  une phrase à « c'est quoi le lien ? ». Une adresse qu'on ne peut
    //  pas dicter au téléphone ne sert à rien.
    //
    //  DÉSORMAIS : /tv, et c'est tout. Téléchargement DIRECT du fichier
    //  — Downloader attend un fichier, pas une page HTML.
    if (segments.length === 1 && segments[0].toLowerCase() === 'tv') {
      return proxyApk(SEVENTV_APK_URL, 'SevenMotionTV.apk',
        url.searchParams.get('v'));
    }

    //  ANCIENNES ADRESSES — elles n'existent plus comme adresses, elles
    //  ne font que renvoyer vers /tv.
    //
    //  POURQUOI ON NE LES SUPPRIME PAS SÈCHEMENT : des box installées en
    //  juin ont ces liens notés sur un papier ou dans les favoris du
    //  Downloader. Un 404 chez un client qui paie, pour gagner huit
    //  lignes de code, est un mauvais échange. Rediriger coûte zéro et
    //  ne laisse personne en rade.
    //
    //  Le résultat est celui demandé : il n'y a plus qu'UNE adresse à
    //  connaître, à écrire et à dicter. Les autres sont des portes qui
    //  mènent à la même pièce.
    if (
      segments.length === 1 &&
      ['defewtv', 'tvbox', 'defew', '777', '7777', 'tv7',
       '7tv', 'seventv', 'sevenmotion', 'apk', 'd', 'direct',
       'install', 'i', 'installer', 'amazon']
        .includes(segments[0].toLowerCase())
    ) {
      return Response.redirect(`${url.origin}/tv`, 301);
    }
