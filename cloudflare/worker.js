...
/// Alias de isPlayableStatus — une seule règle métier (#24 + #25) :
/// abo payé live OU essai encore valide, et PAS gelé / banni / prêté.
/// `st == null` → false (MAC inconnue ≠ fail-open : c'est le trou #24).
function licenseAllowsPlayback(st) {
  return isPlayableStatus(st);
}
...
/// Gel / ban / prêt (loaned) / expiré / pas de fiche → non.
/// Les comptes maîtres passent ailleurs (isMaster), pas ici.
...
//  le risque de fuite).
// =========================================================
//  L'ANCIEN PANEL INTEGRE A ETE SUPPRIME (31/08/2026)
// =========================================================
//  Il vivait ici, en dur, dans un template literal de ~1000 lignes.
//  Le VRAI panel est `admin-panel/` (React + Vite), deploye sur
//  https://tvking-admin.pages.dev — c'est celui que le proprietaire
//  utilise, et il fait tout ce que l'ancien faisait, en mieux.
//
//  POURQUOI LE SUPPRIMER PLUTOT QUE LE LAISSER DORMIR. Deux panels,
//  c'est deux endroits ou coder chaque nouveaute — et la certitude d'en
//  oublier un. C'est exactement ce qui venait d'arriver : les profils
//  famille avaient ete codes ICI, dans l'ecran que personne n'ouvre, et
//  le proprietaire ne les voyait nulle part. Un panel mort qui a l'air
//  vivant coute plus cher qu'un panel absent.
//
//  AVANT DE LE RETIRER, on a verifie route par route ce qu'il savait
//  faire : /admin/clients (liste + fiche + actions), /api/announcement
//  (annonces) et /admin/profiles. Les deux premieres existent dans le
//  panel React (Clients, Appareils, Notifications) ; la troisieme a ete
//  transferee vers /api/v1/profiles/:mac et la page « Profils ».
//  Rien n'a ete perdu.
//
//  L'adresse /admin/panel REDIRIGE desormais vers le vrai panel plutot
//  que de renvoyer une erreur : un favori garde par quelqu'un doit
//  l'amener au bon endroit, pas sur une page morte.
const REAL_ADMIN_PANEL_URL = 'https://tvking-admin.pages.dev';
...
async function handleHeartbeat(request, env, ctx) {
  ...
  if (env.DB) {
    ...
    // #25 : androidId dès l'INSERT (hérite first_seen_at / block_status).
    await ensureD1Device(env, mac, now, body.androidId || '');
    // #24 : android_id AVANT le calcul d'essai — un spoof de MAC ne
    // doit pas récupérer un essai le temps qu'une écriture parte en fond.
...
//
//  La logique vit dans device_profiles.js, partagée avec /api/v1/profiles
//  (le panel). Ici, il ne reste que la couche HTTP.
...
async function handlePublicDeviceSource(env, mac) {
  ...
  if (env.DB && !isMaster) {
    try {
      const st = await familyStatusForMac(env, MAC);
      // #24 denyUsefulContent (no_license / trial_abuse) + #25 loaned.
...
async function handleRequest(request, env, ctx) {
    ...
    // /admin/panel — l'ancien panel intégré a été SUPPRIMÉ (voir le bloc
    // « L'ANCIEN PANEL INTEGRE A ETE SUPPRIME » plus haut). On redirige
    // vers le vrai panel : un favori gardé par quelqu'un doit l'amener au
    // bon endroit, pas sur une page morte.
    // 301 (permanent) : le navigateur retiendra la nouvelle adresse.
    if (segments[0] === 'admin' && segments[1] === 'panel' && segments.length === 2) {
      return Response.redirect(REAL_ADMIN_PANEL_URL, 301);
    }
    ...
    // NOTE : /admin/profiles/:mac a été retiré avec l'ancien panel — il
    // était son seul appelant. Les profils passent désormais par
    // /api/v1/profiles/:mac (jeton du panel React, droits par revendeur)
    // et par /api/device-profiles/:mac (lecture par l'app).
