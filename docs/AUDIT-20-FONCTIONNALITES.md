...
| 3 | Retour en arrière sur le direct (timeshift / catch-up) | ✅ **Fait (via l'archive fournisseur)** | Réalisé SANS enregistrement local ni changement de lecteur : on rejoue depuis l'**archive du fournisseur** (`tv_archive` Xtream / `catchup-days` M3U). `CatchupUrlBuilder` fabrique l'URL timeshift ; le lecteur reçoit une URL comme une autre → qualité/stabilité intactes. **Mobile** : déjà branché (émissions passées). **TV** : AJOUTÉ à DEUX endroits — (a) le guide par chaîne (bouton « ⟲ Revoir » sur émission passée OU en cours), et (b) la GRILLE HORAIRE (timeline) où chaque case d'émission est désormais focusable au D-pad : OK = regarder en direct (émission en cours) ou ⟲ revoir (émission passée avec archive). Le décalage ±30 min passe de flèches cachées à des boutons visibles ⟨ ⟩ → les flèches sont libérées pour naviguer entre les cases. Si la chaîne n'a pas d'archive → pas de pastille ⟲. |
...
| 18 | Téléchargement hors ligne | ✅ **Existe + ENRICHI** | Existait : Enregistrements (REC) du direct. AJOUTÉ : **téléchargement de FILMS** pour le hors-ligne façon Netflix (`VodDownloadService`) — le fichier VOD (déjà compressé par le fournisseur, bien plus léger que le live) est tiré en tâche de fond avec **reprise HTTP Range** (zéro octet re-téléchargé après coupure) + reconnexion auto. Bouton « Télécharger » sur la vedette Films, écran « Mes téléchargements » (Réglages), lecture 100 % hors connexion via le lecteur existant (fichier `file://` → mêmes commandes Netflix ±10 s). Honnêteté technique : pas de « compression magique » — Netflix non plus ; on livre un fichier fini efficacement encodé. |
