...
| 3 | Retour en arrière sur le direct (timeshift) | ⛔ **Écarté** | Exige un buffer DVR côté *fournisseur* du flux (les flux IPTV live n'exposent pas de fenêtre de recul) + réécriture profonde du lecteur = le risque de stabilité maximal pour un gain incertain. Alternative déjà en place : **Enregistrements** (REC). |
