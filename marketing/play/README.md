...
Google a rejeté la première version de la fiche avec « ne décrit pas
clairement ses fonctionnalités ». Ces textes disent donc CE QUE L'APP
FAIT dès le premier mot — pas ce qu'elle est.

**FR** (79 / 80)

```
Ouvrez vos liens M3U ou Xtream et regardez-les : direct, films, séries, EPG.
```

**EN** (78 / 80)

```
Open your own M3U or Xtream links and watch them: live, movies, series, EPG.
```

## Description complète — FR

```
7 MOTION ouvre les liens M3U/M3U8 et les comptes Xtream Codes que VOUS
possédez, et les lit sur votre téléphone, votre tablette et votre Android TV.

CE QUE FAIT L'APPLICATION, CONCRÈTEMENT
1. Vous collez un lien M3U/M3U8, ou vous saisissez un serveur, un nom
   d'utilisateur et un mot de passe Xtream Codes.
2. L'application lit votre liste et la range : direct, films, séries,
   catégories, par pays.
3. Vous ouvrez une entrée, elle se lit en plein écran, avec la reprise là
   où vous vous étiez arrêté, les pistes audio et les sous-titres.

7 MOTION ne fournit, n'héberge, ne vend et n'indexe aucun contenu, aucune
chaîne et aucun abonnement. L'application est VIDE à l'installation : elle
affiche un écran d'accueil qui vous demande votre source, et rien d'autre
n'apparaît tant que vous ne l'avez pas ajoutée.
...
```
7 MOTION opens the M3U/M3U8 links and Xtream Codes accounts that YOU own,
and plays them on your phone, tablet and Android TV.

WHAT THE APP ACTUALLY DOES
1. You paste an M3U/M3U8 link, or enter an Xtream Codes server, username
   and password.
2. The app reads your list and organises it: live, movies, series,
   categories, by country.
3. You open an entry and it plays full screen, resuming where you left
   off, with audio tracks and subtitles.

7 MOTION does not provide, host, sell or index any content, channel or
subscription. The app is EMPTY on install: it shows a welcome screen
asking for your source, and nothing else appears until you add one.
...
Entre 2 et 8, prises sur l'app ACTUELLE, où l'on doit lire **7 MOTION**.
L'ordre compte : Google lit les deux premières.

1. L'écran d'accueil vide (celui qui demande la source) — il prouve à lui
   seul que l'app ne fournit rien.
2. L'écran d'ajout de source, avec les champs M3U ou Xtream visibles.
3. La liste rangée en catégories.
4. Le lecteur en plein écran, barre de progression visible.
5. Facultatif : le guide des programmes, les favoris.

Règle absolue : **aucun logo ni nom de chaîne réelle** à l'écran. Utilise
une source de test aux noms neutres si besoin — une capture avec un
bouquet reconnaissable est le motif de refus le plus fréquent.
